...
SIDE_LENGTH = 20
#Snake color
SNAKE_COLOR = introcs.RGB(100,200,200)
#Box color
...
###General methods
    ...
    def changecolor(self,input):
        ...
        if input.is_key_down('spacebar'):
            ...
            current = False
        change = current>0 and self._colorpress == 0
        if change:
...
####Snake and movement
    ...
    def turnSnake(self):
        ...
        for r in self._snake:
            rpoint = Point(r.x,r.y)
            for a in self._pointlist:
                px = a.x
                py = a.y
                if rpoint.x == px and rpoint.y == py:
                    r._direction = a.d
