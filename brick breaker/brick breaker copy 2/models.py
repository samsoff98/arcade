...
class Circle(GEllipse):
    ...
    def move(self):
            ...
            if self.movex > SPEED_LIMIT_X:
                self.movex = SPEED_LIMIT_X
            if self.movex < -SPEED_LIMIT_X:
                self.movex = -SPEED_LIMIT_X
            if self.movey > SPEED_LIMIT_Y:
                self.movey = SPEED_LIMIT_Y
            if self.movey < -SPEED_LIMIT_Y:
                self.movey = -SPEED_LIMIT_Y
...
class survivor(GImage):
    ...
    is off, you will get 3 lives at the beginning of each level.
    """
    def __init__ (self,left, top, source):
