...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        """
        self._text = GLabel(text="Press s to Start", font_size = 40, left = GAME_WIDTH//3,
    ...
    def update(self,dt):
        ...
        if self._state == STATE_USER_PAUSED:
            self.player_paused_to_continue()
    ...
    def setState(self,s):
        ...
        Precondition: s is an int between 0-5 """
    ...
    def inactive_to_newwave(self):
        ...
        if the s key is pressed and released."""
    ...
    def active_to_paused(self):
        """Changes the state from STATE_ACTIVE to STATE_PAUSED either when the
        p key is pressed or when the ship is hit with an alien bolt."""
    ...
    def player_active_to_paused(self):
        ...
        if change == True:
            self._text = GLabel(text="Game Paused. Press P to continue.", font_size = 30,
            left = GAME_WIDTH/5, bottom = GAME_HEIGHT/3, bold = True,
            font_name = 'Arcade.ttf')
            self._text = GLabel(text = "lives: ", font_size = 20, left = 10,
             bottom = GAME_HEIGHT-10, font_name = 'Arcade.ttf')
            self._state = STATE_USER_PAUSED
        self._LK = current



    def player_paused_to_continue(self):
        self._text = GLabel(text="Game Paused. Press P to continue. \n Up to shoot left and right to move.", font_size = 30,
            left = GAME_WIDTH/5, bottom = GAME_HEIGHT/3, bold = True,
            font_name = 'Arcade.ttf')
    ...
    def endgame(self):
        if self._wave.getGameOver() == 'win':
            self._text = GLabel(text = "YOU WIN! Press s to play again",font_size= 40,font_name= 'Arcade.ttf',
                            x= GAME_WIDTH/2, y= GAME_HEIGHT/2)
            self.restart()
        if self._wave is not None and (self._wave.getGameOver() == 'lost' or self._wave.getLives() == 0):
            self._text = GLabel(text = "GAME OVER. Press s to restart",font_size= 30,font_name= 'Arcade.ttf',
                            x= GAME_WIDTH/2, y= GAME_HEIGHT/2)
            self.restart()
