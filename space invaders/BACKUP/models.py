...
class sound(GImage):
    """
    A class to represent a whether the game sound is on or off, to be displayed
    during STATE_ACTIVE, so the player knows if the game is muted or not.
    """

    def __init__ (self,left,top, source):
        """
        Initializes a sound image.

        This function creates a sound image using the dimensions specified in
        consts.py. It makes an Sound image of width SOUND_WIDTH.
        It places the image at position (x,y) and gives it the image SOUND_IMAGE.

        Parameter left: The position of the left edge of the sound image
        Precondition: left is an int or float

        Parameter top: The vertical coordinate of the top edge of the sound image
        Precondition: top is an int or float

        Parameter source: The file for this image given from SOUND_IMAGE
        Precondition: source must be a str referring to a valid file
        """
        super().__init__(left=left,top=top,width=SOUND_WIDTH,height=SOUND_HEIGHT,
                source=source)

class bomb(GImage):
    """
    A class to represent a single bonb, to be displayed during STATE_ACTIVE, to
    be shot by the ship as a power up.
    """
    def getBombY(self):
        """
    ...
    def getBombVelocity(self):
        """
        Returns the bomb's velocity.
        """
    ...
    def __init__ (self,x,y,source):
        """
        Initializes a single bomb image.

        This function creates a single bomb image using the dimensions specified in
        consts.py. It makes an heart of width BOMB_WIDTH.


        Parameter x: The x position of the bomb
        Precondition: x is a positive int or float

        Parameter y: The y coordinate of the bomb
        Precondition: y is a positive int or float

        Parameter source: The file for this image given from BOMB_IMAGE
        Precondition: source must be a str referring to a valid file
        """
        super().__init__(x=x,y=y,width=BOMB_WIDTH,height=BOMB_HEIGHT,
                source=source)
        self._velocity = BOMB_SPEED
